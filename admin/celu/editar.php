...
if (isset($_GET['ID'])) {
    ...
    $sql = $conexion->prepare("SELECT * FROM celulares WHERE ID = ?");
    $sql->bind_param("i", $id);
    $sql->execute();
    $resultado = $sql->get_result();
    $celulares = $resultado->fetch_assoc();
    ...
    if (isset($_POST['editar'])) {
        ...
        $sql = $conexion->prepare("UPDATE celulares SET
        titulo = ?,
        precio = ?,
        descripcion = ?
        WHERE ID = ?");
...
<div class="card bg-success bg-gradient" style="--bs-bg-opacity: .3;">
    <div class="card bg-success bg-gradient m-5" style="--bs-bg-opacity: .7;">
        <div class="card-header bg-success">
            <h2 class="text-center">Modificar Celular</h2>
        </div>
        <div class="card-body m-2">
            <form action="" method="POST">
                <div class="mb-2">
                    <label for="titulo" class="form-label">Titulo:</label>
                    <input type="text" class="form-control" id="titulo" name="titulo" value="<?php echo $celulares['titulo']; ?>">
                </div>
                <div class="mb-2">
                    <label for="precio" class="form-label">Precio:</label>
                    <input type="text" class="form-control" id="precio" name="precio" value="<?php echo $celulares['precio']; ?>">
                </div>
                <div class="mb-2">
                    <label for="descripcion" class="form-label">Descripción:</label>
                    <input class="form-control" id="descripcion" name="descripcion" value="<?php echo $celulares['descripcion']; ?>">
                </div>
